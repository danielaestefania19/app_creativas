...
#[ic_cdk::update]
fn set_item(item: CreateItem) -> Result<String, ItemError> {
    ...
    Ok("Producto registrado correctamente".to_string())
}
...
fn get_items_owner() -> Result<Vec<(u64, Item)>, ItemError> {
    let caller = Some(ic_cdk::api::caller()); // Envuelve el caller en Some()
