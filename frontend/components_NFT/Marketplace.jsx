            ...
            {assets.map((asset, index) => {
                ...
                return (
                    <div key={index} className="card-container col-span-1">
                        <div className="relative bg-cover bg-center w-full ${showMore ? 'h-auto' : 'h-96'} flex flex-col justify-between rounded-lg overflow-hidden shadow-md bg-white p-4">
                            <h2 className='text-center text-xl font-bold text-black'>Obra/Pza #{ethers.utils.formatUnits(asset.assetId, 0)}</h2>
                            <img className="`w-full ${showMore ? 'h-48' : 'h-32'} object-cover mx-auto mb-2 rounded" src={imageUrl} alt="Card image" />
                            <p className="text-center text-gray-600">Titulo: {asset.titulo}</p>
                            <p className='text-center text-gray-600'>Precio: {ethers.utils.formatUnits(asset.price, 0)} ETH</p>
