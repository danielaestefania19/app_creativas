...
const FetchAllAssets = () => {
    ...
    useEffect(() => {
        // Escucha el evento LogRefundStatus
        contract.on("LogRefundStatus", (assetId, status) => {
            console.log(`Asset ID: ${assetId.toString()}, Refund Status: ${status}`);
        });
        ...
        const fetchAllAssets = async () => {
            ...
            for (let i = 0; i < length; i++) {
                ...
                const isRefundingAllowed = await contract.isRefundingAllowed(asset.assetId);
                console.log(`Asset ID: ${asset.assetId.toString()}, Refunding Allowed: ${isRefundingAllowed}`);
                ...
                const isRefundingAllowed_2 = await contract.isRefundingAllowed(asset.assetId);
                console.log(`Asset ID: ${asset.assetId.toString()}, Refunding Allowed After: ${isRefundingAllowed_2}`);
        ...
        // Recuerda remover el listener cuando el componente se desmonte
        return () => {
            contract.removeAllListeners("LogRefundStatus");
        };
    }, []);
